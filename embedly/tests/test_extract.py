...
from embedly.extract import extract_urls, build_embedly_url
...
class MockRedisTest(object):
    ...
    def setUp(self):
        ...
        mock_app_patcher = mock.patch('embedly.extract.current_app')
        self.mock_app = mock_app_patcher.start()
        self.mock_app.redis_client.get.return_value = None
        self.mock_app.redis_client.set.return_value = None
        self.addCleanup(mock_app_patcher.stop)


class ExtractTest(MockRedisTest, unittest.TestCase):
    ...
    def test_extract_multiple_urls_queried_without_caching(self):
        ...
        extracted_urls = extract_urls(self.sample_urls)

        self.assertEqual(self.mock_app.redis_client.get.call_count, 2)
        self.assertEqual(self.mock_requests_get.call_count, 1)
        self.assertEqual(
            self.mock_requests_get.call_args[0][0],
            build_embedly_url(self.sample_urls),
        )
